...
public:
    //最近上下表面高度，法向和类型
    float ceilH;//天花板高度
    Cc3dVector4 ceilNorm;//天花板法向
    float surfaceH;//表面高度
    Cc3dVector4 surfaceNorm;//表面法向
    int surfaceType;//表面类型
    int ceilType;//天花板类型
    //地面，水面，建筑，移动平台的各表面及法向数据也保存下来
    float groundH;
    float waterH;
    float archSurfaceH;
    float archCeilH;
    ...
    CmobilePlatform*pSurfaceMobilePlatform;//指向当前所在surface的平台，仅surfaceType==SURFACETYPE_MOBILEPLATFORM时有效
    CmobilePlatform*pCeilMobilePlatform;//指向当前所在ceil所属平台，仅ceilType==CEILTYPE_MOBILEPLATFORM时有效
    CenvironmentInfo(){
        ceilH=c3d_INF;
        surfaceH=0;
    ...
    void updateInfo(const Cc3dVector4&pos,float Rc,float Rc_small,const Cground*ground,const Cwater*water,CarchScene*archScene,
        ...
        if(archCeilH<=mobilePlatformCeilH){//如果archCeil低
            ceilH=archCeilH;
            ceilNorm=archCeilNorm;
            ceilType=CEILTYPE_ARCH;
        }else{//如果mobilePlatformCeilH低
            ceilH=mobilePlatformCeilH;
            ceilNorm=mobilePlatformCeilNorm;
            ceilType=CEILTYPE_MOBILEPLATFORM;
        }
        //----比较seabedH,groundH,waterH,archSurfaceH,mobilePlatformSurfaceH几者哪个高，则surfaceH就取谁
        ...
        int imax=0;//surfaceH最高者的索引号
        float Hmax=surfaceHList[0];//surfaceH的最大值
        for(int i=1;i<nSurface;i++){//从1开始
