...
public:
    ...
    vector<Cgridcell*> pdirtyList;//脏格子列表（如果一个格子含有多个model，则此格指针在pdirtyList中出现多次）
    float dx,dy,dz;//每个格子的尺寸
    float xmin,xmax,ymin,ymax,zmin,zmax;//grid范围，应保证被管理的模型均不超出grid范围，超出者将被忽略
    CcollisionGrid(){
    ...
    //nxMax,nyMax,nzMax分别为x,y,z方向上的最大段数
    ...
    //返回未mask且出界的元素列表
    {
        ...
        //----
        vector<Cc3dNode*> outRangeList;//出界列表
        ...
        for(int i=0;i<n;i++){
            if(maskList[i])continue;//跳过屏蔽的model
            ...
            if(I<0||I>Imax||J<0||J>Jmax||K<0||K>Kmax){//如果出界
                //加入到出界列表
    ...
    //必须确保传入的pCollisionBossCellList为空
    {
        ...
        for(int i=I-1;i<=I+1;i++){
            for(int j=J-1;j<=J+1;j++){
                for(int k=K-1;k<=K+1;k++){
                    ...
                    int nModel=(int)grid[i][j][k].pModelList.size();
                    for(int u=0;u<nModel;u++){
                        Cc3dNode*pModel=grid[i][j][k].pModelList[u];
                        pCollisionModelList.push_back(pModel);
                    }
                }
            }
        }
