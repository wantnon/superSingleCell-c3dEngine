...
void reCalNorms_precise_normByVertex(Cc3dMeshData*meshData){
    vector<Cc3dVector4> normList((int)meshData->vlist.size());//顶点法向先存到这里
    //将normList中所有元素清0
    ...
    //计算顶点法向量
    //注意，此时的顶点已经分裂了，只能通过位置是否相同来判断是否为同一顶点
    int nv=(int)meshData->vlist.size();
    for(int i=0;i<nv;i++){
        const int vID=i;
        Cc3dVertex&v=meshData->vlist[vID];
        Cc3dVector3 pos=v.getPos();
        //求哪些三角形包含与v位置相同的顶点，就把这个三角形的法向量积加到此顶点上
        int nIDtri=(int)meshData->IDtriList.size();
        for(int j=0;j<nIDtri;j++){
            Cc3dIDTriangle&IDtri=meshData->IDtriList[j];
            //看IDtri是否包含与v重合的顶点，如果有，就将IDtri的法向量累加到v上
            ...
            if(haveSamePosVert){//IDtri有顶点与v重合
                //求IDtri的法向量
                int ID0=IDtri.vID[0];
                int ID1=IDtri.vID[1];
                int ID2=IDtri.vID[2];
...
Cc3dVector4 softMov(const Cc3dVector4&originPos,//初始位置
                    const Cc3dVector4&tgPos,//目标位置
                    const float percentage//移动百分比
)//软移动，返回结果位置
{
	//将originPos移到tgPos，软跟随效果，不一次到位，只移动percentage的比例
	Cc3dVector4 vec=tgPos-originPos;//从originPos到tgPos的向量
	Cc3dVector4 dvec=vec*percentage;//实际移动向量
	//移动
    Cc3dVector4 rsPos=originPos+dvec;
    return rsPos;
}

float softMov(const float origin,//初始值
              const float tg,//目标值
              const float percentage)//移动百分比

//软移动，返回结果值
{
	//将origin移到tg，软跟随效果，不一次到位，只移动percentage的比例
	float diff=tg-origin;//从originPos到tgPos的差
    float actualMov=diff*percentage;//实际移动量
	//移动
