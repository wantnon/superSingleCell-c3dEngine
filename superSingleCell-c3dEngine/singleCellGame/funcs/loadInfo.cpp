...
void loadInfo_mesh(Cc3dMesh*mesh,const string&meshFilePathShort,const string&texClipPathShort,float scale,Cc3dVector4 offsetVec)
//加载网格
//scale为放缩系数，可以在导入时对顶点坐标进行等比方缩
//offsetVec为偏移量（缩放以后的偏移量），可在导入时对顶点坐标进行偏移
//由于是等比放缩，所以法线和纹理坐标不受影响
{
    ...
    assert(mesh->getMeshData()->isEmpty());
    //------------------获得纹理文件夹相对路径
    string meshFileName_part1;//相对路径名，不包含文件名
    string meshFileName_part2;//文件名
    ...
    //------------------提取文件信息到mesh
    {
        ...
        int nv;
        fscanf(fp, "%d",&nv);
        mesh->getMeshData()->vlist.reserve(nv);
        //提取各顶点
        for(int i=0;i<nv;i++){
            ...
            //将此顶点加入vlist
            mesh->getMeshData()->vlist.push_back(vertex);
        }
        //提取IDtri数
        int nIDtri;
        fscanf(fp, "%d",&nIDtri);
        mesh->getMeshData()->IDtriList.reserve(nIDtri);
        ...
        for(int i=0;i<nIDtri;i++){
            ...
            mesh->getMeshData()->IDtriList.push_back(IDtri);
        }
        //提取纹理图片名并加载纹理
        {
            ...
            //根据纹理图片路径加载纹理，并将纹理句柄存入mesh
            mesh->setTexture(Cc3dTextureCache::sharedTextureCache()->addImage(texPathName.c_str()));
        }
        //提取backVisible
        char buffer[1024]={0};
        int backVisible;
        ...
        if(backVisible){
            mesh->setCullFace(ec3dNotCull);
        }else{
            mesh->setCullFace(ec3dCullBack);
        }
...
void loadInfo_model(Cc3dModel*model,const string&modelClipPathShort,float scale,Cc3dVector4 offsetVec){//输入模型文件夹短路径名
    if(modelClipPathShort.empty()==false)assert(modelClipPathShort[(int)modelClipPathShort.size()-1]!='/');
  //  model->modelClipPathShort=modelClipPathShort;
    //----加载各mesh_normByFace
    if(Cc3dFileUtils::sharedFileUtils()->getIsFileOrClipExist(modelClipPathShort+"/mesh")){
        ...
        while (1) {
            ...
            loadInfo_mesh(pmesh,meshFilePathShort,texClipPathShort,scale,offsetVec);
            model->addMesh(pmesh);
            i++;
        }
    ...
    if(Cc3dFileUtils::sharedFileUtils()->getIsFileOrClipExist(modelClipPathShort+"/mesh_normByVertex")){
        ...
        while (1) {
            ...
            loadInfo_mesh(pmesh, meshFilePathShort,texClipPathShort,scale,offsetVec );
            model->addMesh(pmesh);
            reCalNorms_precise_normByVertex(pmesh->getMeshData());
        //    pmesh->setMeshData());
            i++;
        }
    }
