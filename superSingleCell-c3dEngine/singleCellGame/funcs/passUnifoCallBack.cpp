...
void passUnifoCallback_diffuse_ambient_noTransf_shadowMap(Cc3dNode*node, Cc3dProgram*program){
    ...
    Cc3dMatrix4 projMat=mesh->getCamera()->calculateProjectionMat();
    Cc3dMatrix4 viewMat=mesh->getCamera()->calculateViewMat();
    Cc3dVector4 diffuseM=mesh->getMaterial()->getDiffuse();
    assert(mesh->getLight());
    Cc3dVector4 diffuseL=mesh->getLight()->getDiffuse();
    Cc3dVector4 ambientM=mesh->getMaterial()->getAmbient();
    Cc3dVector4 ambientL=mesh->getLight()->getAmbient();
    //
    ...
    Cc3dVector3 lightPos=mesh->getLight()->getPos().toV3();
    //
    Cc3dMatrix4 worldToLightViewportTexCoord=calculateWorldToViewportTexCoordMatrix(mesh->getLight()->getLightViewCameraByIndex(0)->calculateViewMat(),
                                                                               mesh->getLight()->getLightViewCameraByIndex(0)->calculateProjectionMat());
    Cc3dMatrix4 worldToLightViewportTexCoord2=calculateWorldToViewportTexCoordMatrix(mesh->getLight()->getLightViewCameraByIndex(1)->calculateViewMat(),
                                                                                mesh->getLight()->getLightViewCameraByIndex(1)->calculateProjectionMat());
...
void passUnifoCallback_diffuse_ambient_noTransf_noSelfShadow(Cc3dNode*node, Cc3dProgram*program){
    ...
    Cc3dMatrix4 modelMat=mesh->getIsIgnorTransform()?unitMat():Cc3dModelMatStack::sharedModelMatStack()->getTopMat();
    Cc3dMatrix4 projMat=mesh->getCamera()->calculateProjectionMat();
    Cc3dMatrix4 viewMat=mesh->getCamera()->calculateViewMat();
    Cc3dVector4 diffuseM=mesh->getMaterial()->getDiffuse();
    assert(mesh->getLight());
...
void passUnifoCallback_diffuse_ambient_noSelfShadow(Cc3dNode*node, Cc3dProgram*program){
    ...
    Cc3dVector4 ambientM=mesh->getMaterial()->getAmbient();
    Cc3dVector4 ambientL=mesh->getLight()->getAmbient();
    Cc3dVector4 specularL=mesh->getMaterial()->getSpecular();
    Cc3dVector4 specularM=mesh->getLight()->getSpecular();
    //
    Cc3dMatrix4 PVMmat=projMat*viewMat*modelMat;
...
void passUnifoCallback_diffuse_ambient_specular_noSelfShadow(Cc3dNode*node, Cc3dProgram*program){
    ...
    Cc3dVector4 ambientML=componentProduct(ambientM, ambientL);
    Cc3dVector4 specularML=componentProduct(specularM, specularL);
    float shininess=mesh->getMaterial()->getShininess();
    Cc3dVector3 lightPos=mesh->getLight()->getPos().toV3();
    //
    Cc3dMatrix4 worldToLightViewportTexCoord=calculateWorldToViewportTexCoordMatrix(mesh->getLight()->getLightViewCameraByIndex(0)->calculateViewMat(),
                                                                               mesh->getLight()->getLightViewCameraByIndex(0)->calculateProjectionMat());
    ...
    program->passUnifoValueNfv("eyePos_world",mesh->getCamera()->getEyePos().getArray(),3);
}
...
void passUnifoCallback_noLight(Cc3dNode*node, Cc3dProgram*program){
    ...
    //
    Cc3dMatrix4 PVMmat=projMat*viewMat*modelMat;
    Cc3dVector4 color=mesh->getMaterial()->getColor();
