...
bool CsuccedPage::init(int nStar,int nEnemyKilled,int nCoinEaten){
    ...
    //--quad_succeed
    {
        ...
        m_quad_succeed->getModelList()[0]->getMeshByIndex(0)->getIndexVBO()->genBuffers();
        m_quad_succeed->getModelList()[0]->getMeshByIndex(0)->setTexture(ptexture_quad_succeed);
        ...
        m_quad_succeed->getModelList()[0]->getMeshByIndex(0)->submit(GL_STATIC_DRAW);
        //
    ...
    //--quad_star
    {
        ...
        m_quad_star->getModelList()[0]->getMeshByIndex(0)->submit(GL_STATIC_DRAW);
        //
    ...
    //--nextLevelKey
    {
        ...
        m_nextLevelKey->getModelList()[0]->getMeshByIndex(0)->setTexture(ptexture);
        ...
        m_nextLevelKey->getModelList()[0]->getMeshByIndex(0)->submit(GL_STATIC_DRAW);
        //
    ...
    //----quad_nCoinPositivePlaceList
    {
        ...
        for(int i=0;i<nCoinPositivePlaceCount;i++){//反序
        ...
        for(int i=0;i<nPositivePlace;i++){
            ...
            quad->getModelList()[0]->getMeshByIndex(0)->setTexture(m_numberTexList[numberList[i]]);
...
void CsuccedPage::update(){
    ...
    if(m_nextLevelKey->isContainPoint(Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getPoint())&&Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getIsTapOnce()){
        if(CgameState::sharedGameState()->getCurrentLevel()==CuserConfig::sharedUserConfig()->get_levelCount()){//通过了最后一关
    ...
    //nextLevelKey响应按下和抬起
    if(Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getIsDown()&&m_nextLevelKey->isContainPoint(Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getPoint()) ){
        m_nextLevelKey->useKeyDownColor();
    }else{
        m_nextLevelKey->useKeyUpColor();
    }
    if(CgameState::sharedGameState()->getCurrentLevel()!=CuserConfig::sharedUserConfig()->get_levelCount()){//不是最后一关才响应下面按键
        //----homeKey
        //homeKey响应单击
