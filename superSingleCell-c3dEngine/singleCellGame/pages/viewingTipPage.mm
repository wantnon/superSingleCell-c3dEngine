...
bool CviewingTipPage::init(){
    ...
    //--quad_tipTex
    {
        ...
        m_quad_tipTex->genAndInit(bx, bx+w, by, by+h,0,1,0,1);//不考虑分辨率
        m_quad_tipTex->getModelList()[0]->getMeshByIndex(0)->getIndexVBO()->genBuffers();
        m_quad_tipTex->getModelList()[0]->getMeshByIndex(0)->setTexture(texture);
        ...
        m_quad_tipTex->setVisitOrder(10000);
        //
        m_quad_tipTex->reSetCenter(width/2,height/2);
        m_quad_tipTex->getModelList()[0]->getMeshByIndex(0)->submit(GL_STATIC_DRAW);
        //
