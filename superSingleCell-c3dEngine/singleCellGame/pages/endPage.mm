...
bool CendPage::init(){
    ...
    //--quad_endPage_backGround
    {
        ...
        m_quad_endPage_backGround->getModelList()[0]->getMeshByIndex(0)->getIndexVBO()->genBuffers();
        m_quad_endPage_backGround->getModelList()[0]->getMeshByIndex(0)->setTexture(ptex);
    ...
    //----quad_waitingForMusicToStop
    {
        ...
        m_quad_waitingForMusicToStop->getModelList()[0]->getMeshByIndex(0)->submit(GL_STATIC_DRAW);
        //
...
void CendPage::update(){
    //----homeKey
    if(this->getALSourceByName("endMusic")->getIsPlaying()==false){//如果音乐停止则响应按键
        //homeKey响应单击
...
void CendPage::draw(){
    if(this->getALSourceByName("endMusic")->getIsPlaying()==false){//如果音乐停止则显示按钮
