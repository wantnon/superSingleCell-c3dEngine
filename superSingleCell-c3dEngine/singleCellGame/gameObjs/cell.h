...
//model0:核，model1:线粒体，model2:外壳，model3:腿，model4:胞体
...
{
public:
    bool isPuddleWater;//是否在划水
    bool isMovingOnSolidSurface;//是否在固体表面移动
public:
    ...
    float cos_A_wing;
    float sin_A_wing;
    bool wingIsMovingUp;//wing是否在扬起
    float vup;//竖直速度(每帧竖直位移量)
    float a_gravity;//重力加速度(每帧竖直位移改变量)
    Ccur_foe<bool> isJump;//是否已起跳
    Cc3dMatrix4 RTmat_store;//存储RTmat，用于恢复
    Clife life;//生命
    Clife fuel;//燃料
    Ccur_foe<float> nEateCoin;//吃金币数量(充许出分数)
    Ccur_foe<int> nKillEnemy;//杀敌人数量
    float Rc_small;//中心到底或顶的距离（半短轴长度）
    float Rc_small_value_flat;//摔扁时中心悬起高度
    float Rc_small_value_normal;//正常时中心悬起高度
    float H_causeFlat;//摔扁所需高度
    bool isFallFlat;//摔扁
    long flatTimeCount;//摔扁时间计数
    Ccur_foe<float> jumpH_landH;//起跳高度和落地高度
    int nkey;//多少个钥匙过关
    int key_got;//已获得了多少个钥匙
    Cship*pShip_latest;//最近驾驶的ship（如果当前pShip不为NULL，则pShip_latest==pShip）
    float H_sinkInWater;//吃水深度
    bool rotLegs;//是否旋转腿
    float xmin,xmax,ymin,ymax,zmin,zmax;//活动范围
    bool isFly;//是否飞行
    float v0up_basic;//基本竖直起跳速度
    bool isBouncing;//是否处于自动弹跳状态（一但被加速）
    bool bouncingAlreadyAccelerated;//自动弹跳已被加速
    CtimeCounter revTimeCounter_bouncingReceiveAccelerate;//自动弹跳接受加速倒计时
    float dH_autoJump;//高度差多大时自动转为跳起状态（用于实现从墙头自然落下的效果）
    bool onFire;//是否着火
    CtimeCounter timeCounter_onFire;//onFire计时
    int nGun;//子弹路数
    CtimeCounter revTimeCounter_shot;//射击结束倒计时
    bool isShotTimeLastsLong;//是否是长延时
    bool shotBallTrace;//是否进行子弹追踪
    float deathY;//低于此值死亡
    //----控制信号
    bool rotSignal;
    bool jumpSignal;
...
protected:
    bool m_isDied;
    vector<CshotBall*> pshotBallList;//子弹列表
...
public:
    Ccell(){
        ...
        cos_A_wing=cosf(0);
        wingIsMovingUp=true;
        nEateCoin.setFoe(-1);//为使isChange初始为true，从而迫使界面更新
        nKillEnemy.setFoe(-1);//为使isChange初始为true，从而迫使界面更新
        v_legsRot=0;
...
/*    void addMolecule(Cmolecule*molecule){
    ...
    //调整位姿(软调整)，使局部坐标系的Y轴与世界空间中norm一致,norm须为单位向量
    //k越小越软，k最大为1，为1时即硬调整
    {
        Cc3dMatrix4 RTmat=this->getTransform().getRTmat();
        //--------------------
        bool isSoft=(k!=1.0);//是否为软调整
        Cc3dVector4 Xvec,Zvec;//局部空间X轴和Z轴的世界向量
        Zvec.init(RTmat.getAt(8),RTmat.getAt(9),RTmat.getAt(10),0);//将RTmat[8~10]拷给Zvec，即取得局部空间z轴的世界向量（Zvec已为单位向量）
        if(fabsf(dot(Zvec, norm)-1)>0.01){//如果Zvec与norm不平行
            //根据norm和Zvec计算新X轴
            ...
            //重新计算Z轴
            Zvec=cross(Xvec,norm);//由于Xvec和norm垂直且都是单位向量，所以Zvec必是单位向量，不用再单位化
        }else{//如果Zvec与norm平行
            //改用X轴进行计算
            Xvec.init(RTmat.getAt(0),RTmat.getAt(1),RTmat.getAt(2),0);//将RMmat[0~2]拷给Xvec，即取得局部空间x轴世界向量
            //根据norm和Xvec计算新Z轴
            Zvec=cross(Xvec,norm);//Xvec和norm虽然都是单位向量，但不一定垂直所以Zvec不一定是单位向量
            //将Zvec单位化（如果是软调整，则由于后面要统一进行规范正交化，所以此处就可以省去规范化）
            if(!isSoft)Zvec=normalize(Zvec);
            //重新计算X轴
            Xvec=cross(norm,Zvec);//由于norm与Zvec垂直，且二者都是单位向量，所以Xvec是单位向量，不用再单位化
        }//得到Xvec,norm,Zvec
        //由Xvec,norm,Zvec构成新的坐标系，而平移分量仍取RTmat的
        //制作新的位姿矩阵
        Cc3dMatrix4 mat(
            Xvec.x(),Xvec.y(),Xvec.z(),0,//第一列
            norm.x(),norm.y(),norm.z(),0,//第二列
            Zvec.x(),Zvec.y(),Zvec.z(),0,//第三列
            RTmat.getAt(12),RTmat.getAt(13),RTmat.getAt(14),1//第四列
        );
        //软插值
        if(isSoft){
            ...
            //以上用简单的线性插会导致矩阵不再正交，所以此时要进行一下正交化
            RTmat=orthogonalization3x3(RTmat);
        }else{
            RTmat=mat;
        }
        //--------------------------
        this->getTransformPointer()->setRTmat(RTmat);
    ...
    //假设局部dir是(0,0,-1)
    {
        ...
        up=this->getTransform().convertToWorldSpace(up);//up已转为世界坐标
        //根据dir和up计算right
        Cc3dVector4 right=cross(dir, up);//虽然dir和up都是单位向量，但二者不一定垂直，所以right不一定是单位向量
        if(!isSoft)right=normalize(right);//right单位化（如果是软调整，则由于后面要统一进行规范正交化，所以此处就可以省去规范化）
        //根据right和dir重新计算up
        up=cross(right, dir);//由于right与dir垂直且都为单位向量，所以得到的up必为单位向量
        //将right作为X轴,up作为Y轴，-dir作为Z轴建立直角坐标系
        //生成此坐标系的矩阵，同时将原来的RTmat的平移分量拷过来
        Cc3dMatrix4 mat(
            right.x(),right.y(),right.z(),0,//第一列
            up.x(),up.y(),up.z(),0,//第二列
            -dir.x(),-dir.y(),-dir.z(),0,//第三列
            RTmat.getAt(12),RTmat.getAt(13),RTmat.getAt(14),1//第四列
        );
        //软插值
        if(isSoft){
    ...
    void accelerateBouncing(){
        //如果是处于自动弹跳中且还未被加速过且刚开始起跳不久，则可以被加速
    ...
    void waveWing(){
        if((int)this->getModelByIndex(CELLPART_wingL)->getMeshCount()==0
           &&(int)this->getModelByIndex(CELLPART_wingR)->getMeshCount()==0)return;
        if(wingIsMovingUp){
            sin_A_wing+=0.15;
            if(sin_A_wing>0.5){
        ...
        this->getModelByIndex(CELLPART_wingL)->getTransformPointer()->setRot(0, 0, 1, cos_A_wing, sin_A_wing);
        this->getModelByIndex(CELLPART_wingR)->getTransformPointer()->setRot(0, 0, 1, cos_A_wing, -sin_A_wing);
    }
    ...
    CmobilePlatform* get_pInertialSurfaceMobilePlatform()const {
        ...
        if(isJump.get()==false){//如果落到surface上
            if(environmentInfo.surfaceType==SURFACETYPE_MOBILEPLATFORM){//如果落在了移动平台上
    ...
    void submitModels(GLenum usage)//提交各model数据
    {
        submitVertex(usage);
        submitIndex(usage);
    }
    ...
    void doJump_bounce(float _vup){//自动弹跳
        doJump(_vup);
        isBouncing=true;
    ...
    void endJumping(){
        vup=-0.01;//不是归0，而是给一个很小的向下的速度，以便着陆更稳，尤其是在运动平向上时
        isJump.set(false);
        isBouncing=false;
    }
    ...
    float get_vupInertial_causedByMobilePlatform(){//获得在移动平台影响下产生的竖直方向上的惯性速度
    ...
    void doTriggerCeilPlatform()const
    {
        if(isTouchCeil(1.0)){
    ...
    void doTriggerSurfacePlatform()const {
    ...
    void doRiseAndFall()//升起和下降
    //triggerPlatform指示是否触发移动平台
    {
        updateJumpingCurFoe();
        ...
        //判断是否接触天花板
        if(isTouchCeil(0)){
            isJump.set(true);//如果不处于跳起状态，也人为令其为跳起状态
            vup=-0.15;//为了不与天花板发生“粘黏”，不是将vup设为0，而是设为一个负值（即给一个向下的初速度）
            m_transform.setPosY(environmentInfo.ceilH-Rc_small);//让Rc_small上沿正好与天花板重合，注意，一定要正好重合，千万不要加margin，否则会对其它地方的isTouchCeil()的判断产生影响
            Cc3dVector4 norm=Cc3dVector4(0,0,0,0)-environmentInfo.ceilNorm;//取CeilNorm的负值
        ...
        switch(environmentInfo.surfaceType){
            ...
            {
                if(isTouchSurface(1.0)){//接触表面
                    endJumping();
                    jumpH_landH.set(m_transform.getPosY());
                    //自动弹跳
                    CmobilePlatform*pPlatform=environmentInfo.pSurfaceMobilePlatform;
                    if(pPlatform!=NULL&&pPlatform->get_isBouncing()&&isJump.get()==false){//是弹跳平台
                        //跳起
    ...
    bool isTouchSurface(const float hmargin)const
    //目前在此游戏中hmargin大部分情况统一取1.0，个别特殊情况可能取得更大些
    //要注意保持一致性
    {//判断是否接触表面
        //有hmargin，使得touchSurface的判断不容易miss，避免穿越表面
        return this->getTransform().getPos().y()-this->Rc_small-hmargin<=environmentInfo.surfaceH;

        
    }
    bool isTouchCeil(const float hmargin)const {//判断是否接触天花板
        //有hmargin，使得touchSurface的判断不空易miss，避免穿越天花板
    ...
    //前端返回是否接触表面
    {
		...
		if(isTouchSurface(margin+dH_autoJump)==false){//如果此时还没有接触地面，则恢复跳跃状态（自由落体）
            isJump.set(true);//这个动作非常重要，是使得主角能从墙头正确自然下落的关键
		}else{
			//根据topSurface决定不同的调整动作
			switch(environmentInfo.surfaceType){
                case SURFACETYPE_MOBILEPLATFORM:
                case SURFACETYPE_ARCH:
				{ //着陆到建筑表面
                    //调整模型高度
                    m_transform.getRTmatPointer()->setAt(13, environmentInfo.surfaceH+Rc_small+margin);
                    //调整姿态
                ...
                case SURFACETYPE_GROUND:
				{
                        downToSurface_soft(Rc_small,environmentInfo.surfaceH+margin,
                                           environmentInfo.surfaceNorm,k_ground_H, k_ground_posture ) ;//主角软着陆到地面
                }
                    break;
                case SURFACETYPE_WATER:
				{
    ...
    //调整高度用软调整可以避免严格贴地行走，实现一定飘逸
	{
        ...
        if(isTouchSurface(margin+dH_autoJump)==false){//如果此时还没接触地面，则回到跳跃状态（自由落体）
            ...
            switch(environmentInfo.surfaceType){
                ...
                {
                    downToSurface_soft(Rc_small,environmentInfo.surfaceH+margin,
                                       environmentInfo.surfaceNorm, k_ground_H, 1,false) ;//软着陆到地面，但不调整姿态
                }
                    break;
                case SURFACETYPE_WATER:
                {
                    downToSurface_soft(Rc_small,environmentInfo.surfaceH+margin-H_sinkInWater,
                                       environmentInfo.surfaceNorm, k_water_H,1,false);//软着陆到水面，但不调整姿态
                }
                    break;
            }
    ...
    {
        if(d<=0)return;
        //恢复平移分量，不恢复旋转分量，因为允许旋转
        m_transform.getRTmatPointer()->setAt(12, posf.x());
        m_transform.getRTmatPointer()->setAt(13, posf.y());
        m_transform.getRTmatPointer()->setAt(14, posf.z());
        Cc3dVector4 back;
        float d_back=d*0.35;//0.25;//d*0.1//除了恢复原位外另加一个回退
        back=e_back*d_back;
        back.sety(0);
    ...
    void collisionReAct_slip(float d,const Cc3dVector4&e_back){
        ...
        //恢复平移分量，不恢复旋转分量，因为允许旋转
        m_transform.getRTmatPointer()->setAt(12, this->RTmat_store.getAt(12));
        m_transform.getRTmatPointer()->setAt(13, this->RTmat_store.getAt(13));
        m_transform.getRTmatPointer()->setAt(14, this->RTmat_store.getAt(14));
        Cc3dVector4 back;
        float d_back=d*0.35;//0.25;//0.1;//除了恢复原位外另加一个回退
        back=e_back*d_back;
        back.sety(0);
        m_transform.moveRelativeToFather(back.x(),back.y(),back.z());
        //----滑动
        ...
        if(dot(movVecXZ,e_back)<=0)
            //movVecXZ在e_back上投影为负时才滑动
            //这是为了避免墙角处由于e_back计算误差而造成滑向墙壁的另一面
        {
            //求e_back在XZ平面上的垂直向量
            Cc3dVector4 e_tangentXZ(-e_back.z(),0,e_back.x(),0);
            //求moVecXZ在e_tangentXZ的投影
            float dot_movVecXZ_e_tangentXZ=dot(movVecXZ, e_tangentXZ);
            //求受限移动向量在XZ上的投影
            Cc3dVector4 movVecRestrictXZ=e_tangentXZ*(dot_movVecXZ_e_tangentXZ*k_slip);//乘以一个因子是为了减慢滑动速度
            //按movVecRestrictXZ进行移动
    ...
    void legsRot(){//腿旋转
        ...
        getModelByIndex(CELLPART_legs)->getTransformPointer()->rotate(0, 1, 0, cos_v_legsRot, sin_v_legsRot);
    }
    void doOnFireTimeCount(){
        if(onFire==false)return;
        //计时
        if(timeCounter_onFire.getCount()>onFireDuringTime){//如果已超过持续时间
            onFire=false;//onFire消失
            timeCounter_onFire.reSet();//重置计时器
        }else{//如果未超过持续时间
            //计时
    ...
    void loadConfig(const string&fileNameWithExt){
        ...
        //------------------提取文件信息，并建立列模型列表
        {
            ...
            m_transform.getRTmatPointer()->init(initRmat);
            m_transform.getRTmatPointer()->setAt(12, initPos[0]);
            m_transform.getRTmatPointer()->setAt(13, initPos[1]);
            m_transform.getRTmatPointer()->setAt(14, initPos[2]);
            //提取fuelValue
            float fuelValue;
            fscanf(fp, "%s",buffer);
    ...
    void shotBall_mov(/*const Cwater&water*/){//子弹移动
        //获得cell位置
        ...
        for(int i=0;i<nShotBall;i++){
            ...
            //移动前存储位置
            pball->storePos();
            //对于指向敌人的pball，判断pball指向的敌人是否已死，如果已死则将pball->ptgBoss置空
            if(pball->ptgBoss!=NULL&&((Ccell*)pball->ptgBoss)->getIsDied()){
                pball->ptgBoss=NULL;
            }
            //移动
            {
                if(pball->ptgBoss==NULL){//如果ptgBoss为空
                    //前进
                    pball->getTransformPointer()->moveZ( -v_shotBall);
                    ...
                    pball->getTransformPointer()->setRmat(Rmat);
                    //前进
                    pball->getTransformPointer()->moveZ( -v_shotBall);
                    pball->pathLen_add(v_shotBall);
                }
    ...
    //pFlyLiveBossList为活着的飞行boss列表
    {
        ...
        for(int i=0;i<nGun;i++){
            ...
            float cosA=cosf(A);
            float sinA=sinf(A);
            //从pshotBallList中找一个active==false且shotBallTypeID与传入的shotBallTypeID相匹配的元素
            ...
            //为pshotBall定位
            //先让其与cell方位一致
            pshotBall->getTransformPointer()->setRTmat(this->getTransform().getRTmat());
            //旋转一个角度
            pshotBall->getTransformPointer()->rotate(0, 1, 0, cosA, sinA);
            //调整高度
            pshotBall->getTransformPointer()->moveY(5);//2.3//1.6//1.3
            //求pshotBall->ptgBoss
            if(shotBallTrace){//如果追踪
                Cc3dVector4 dir=pshotBall->getDir();//pshotBall的单位方向向量
                //看pshotBall的射线与哪个敌人最近，就让其ptgBossCell就指向谁
                ...
                for(int i=0;i<nLiveBoss;i++){
                    ...
                    float d2_toThis=getLength2(pboss->getTransform().getPos()-this->getTransform().getPos());//boss到this的距离
                    if(d2_toThis>shotBallReachDis*shotBallReachDis)continue;
                    float d2_toLine=d2_pointToRay(pshotBall->getTransform().getPos(), pshotBall->getDir(), pboss->getTransform().getPos());//boss到子弹射线的距离
                    if(d2_toLine<d2_toThis*k2+10){
                        if(giveUpTraceIfBossIsGroundBossAndMeIsNotJumpingAndShotBallNeedToBendDown&&isJump.get()==false&&pboss->get_isFly()==false){//如果对地面敌人在需要向下转弯的情况下放弃追踪，且pboss恰好是地面敌人
                            //判断追踪pboss是否需要向下转弯
                            Cc3dVector4 _dir=pboss->getTransform().getPos()-pshotBall->getTransform().getPos();
                            _dir=normalize(_dir);
                            //看dir到_dir是否要向下转
                            if(dir.y()>_dir.y()+0.01){//需要向下转
                                //不合要求
                                continue;
                            }
                        }
                ...
                //看ptgBossList中哪个离this最近，就令pshotBall->ptgBoss等于谁
                float mind2=c3d_INF;
    ...
    void movCellAndShotBallsWithInertMobilePlatform(){
        ...
        if(pInertialSurfaceMobilePlatform){
            if(isJump.get()==false){//如果没有挑起
                ...
                //但cell所具有的平移速度要比v_inertPlatform小些
                float v_inertCell[3]={0.5*v_inertPlatform[0],0,0.5*v_inertPlatform[2]};//注意，竖直不作位移，因为已在跳起时增加了初速度
            ...
            //所有的shotBall受到平移速度影响
            {
                ...
                for(int i=0;i<nShotBall;i++){
                    ...
                    if(pball->getIsActive()){
                        ...
                        pball->getTransformPointer()->moveRelativeToFather(v_inertPlatform[0], v_inertPlatform[1], v_inertPlatform[2]);
                    }
                }
            }
        }
    ...
    //必须放在着陆之后，否则容易发生与地面之下的墙面进行误碰撞
    //前端返回是否碰撞
    //triggerCollisionPlatforms指示是否将发生碰撞的移动平台触发
    {
        if(multiPoint){
...
private:
    ...
    //triggerCollisionPlatforms指示是否将发生碰撞的移动平台触发
    {
        ...
        //与mobilePlatformSet中各platform进行碰撞检测及碰撞反应
        ...
        for(int i=0;i<n;i++){
            ...
            if(pmobilePlatform->getIsDied())continue;//跳过死亡的运动平台
...
private:
    ...
    //surfaceH和surfaceNorm分别为表面的高度和法向量
    //H0为中心相对于表面悬起高度
    {
        //调整物高度
        Cc3dVector4 pos=m_transform.getPos();
        float dH=(surfaceH+H0-pos.y())*k_H;//软移动量
