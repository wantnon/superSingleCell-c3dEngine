...
public:
 ...
 /*   void set_invisible(bool _invisible){
    ...
    void reSetWidth(float _width){//重设宽度
        xmax=xmin+_width;
        //更新顶点坐标(1号顶点和2号顶点)
        Cc3dVertex&v1=getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist[1];
        Cc3dVertex&v2=getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist[2];
    ...
    void reSetHeight(float _height){//重设高度
        ...
        //更新顶点坐标(0号顶点和1号顶点)
        Cc3dVertex&v2=getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist[2];
        Cc3dVertex&v3=getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist[3];
        Cc3dVector3 pos2=v2.getPos();
    ...
    void reSetCenter(float x,float y){//重设位置
        ...
        ymax+=dy;
        //更新顶点
        int nv=(int)getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist.size();
        for(int i=0;i<nv;i++){
            Cc3dVertex&v=getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist[i];
    ...
    void genAndInit(
                   ...
                   assert((int)this->getModelCount()==0);
        xmin=_xmin;
        xmax=_xmax;
        ymin=_ymin;
        ymax=_ymax;
        ...
        Cc3dMesh*pmesh=new Cc3dMesh();
                   pmesh->autorelease();
                   pmesh->init();
        this->getModelByIndex(0)->addMesh(pmesh);
        ...
        getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist.resize(4);
        memcpy(&(getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->vlist[0]), vertexArray, sizeof(vertexArray));
        getModelByIndex(0)->getMeshByIndex(0)->getMeshData()->IDtriList.clear();
        getModelByIndex(0)->getMeshByIndex(0)->addIDtri(0, 1, 2);
        getModelByIndex(0)->getMeshByIndex(0)->addIDtri(0, 2, 3);
    }
    ...
    void useKeyDownColor(){
        this->getModelByIndex(0)->setColor(keyDownColor[0],keyDownColor[1],keyDownColor[2],keyDownColor[3]);
        
    }
    void useKeyUpColor(){
        this->getModelByIndex(0)->setColor(keyUpColor[0],keyUpColor[1],keyUpColor[2],keyUpColor[3]);
    }
