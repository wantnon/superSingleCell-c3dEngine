...
public:
    ...
    void init(float _rx,float _ry,float sRepeat=1,float tRepeat=1){
        rx=_rx;
        ry=_ry;
        //生成model
        ...
        pmesh->autorelease();
        pmesh->init();
        pmesh->getMeshData()->vlist.resize(nPoint);
        memset(&(pmesh->getMeshData()->vlist[0]),0,sizeof(Cc3dVertex)*nPoint);//清零
        Cc3dIDTriangle IDtri0,IDtri1;
        IDtri0.setvID(0, 1, 2);
        IDtri1.setvID(0, 2, 3);
        pmesh->getMeshData()->IDtriList.push_back(IDtri0);
        pmesh->getMeshData()->IDtriList.push_back(IDtri1);
        Cc3dVertex&v0=pmesh->getMeshData()->vlist[0];
        Cc3dVertex&v1=pmesh->getMeshData()->vlist[1];
        Cc3dVertex&v2=pmesh->getMeshData()->vlist[2];
        Cc3dVertex&v3=pmesh->getMeshData()->vlist[3];
    ...
    void adjustFaceAndSubmitVertex(const Cc3dVector4&eyePos){
        adjustFace(eyePos);
        this->getModel()->getMeshByIndex(0)->submitVertex(GL_STREAM_DRAW);
    ...
    //计算出一个以zvec为z轴的空间直角坐标系
    //zvec须为单位向量
    //返回坐标系鼍两个轴的单位向量xvec,yvec
    {
        Cc3dVector4 up(0,1,0,0);//up朝上
        xvec=cross(up, zvec);
        xvec=normalize(xvec);
    ...
    void adjustFace(const Cc3dVector4&eyePos){//调整朝向(旋转中心在中心)
        //以pos为原点，normalize(pos->eyePos)为z轴建立一个直角坐标系，up朝上
        ...
        Cc3dVertex&v0=this->getModel()->getMeshByIndex(0)->getMeshData()->vlist[0];
        Cc3dVertex&v1=this->getModel()->getMeshByIndex(0)->getMeshData()->vlist[1];
        Cc3dVertex&v2=this->getModel()->getMeshByIndex(0)->getMeshData()->vlist[2];
        Cc3dVertex&v3=this->getModel()->getMeshByIndex(0)->getMeshData()->vlist[3];
        v0.setPos(p0.toV3());
        v1.setPos(p1.toV3());
        v2.setPos(p2.toV3());
        v3.setPos(p3.toV3());
