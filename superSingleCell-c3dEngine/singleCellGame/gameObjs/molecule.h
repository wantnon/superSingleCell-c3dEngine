...
//分子作用
#define MOLECULE_FUNC_noFunc 0 //无作用
#define MOLECULE_FUNC_lifeAdd 1 //加生命
#define MOLECULE_FUNC_noMatch 2 //无敌
#define MOLECULE_FUNC_noShotBall 3 //失去射击能力
#define MOLECULE_FUNC_keyAdd 4 //增加钥匙
#define MOLECULE_FUNC_blast_and_lifeDec 5 //爆炸并掉血
#define MOLECULE_FUNC_transport 6 //传送
#define MOLECULE_FUNC_coinAdd 7 //加金币
#define MOLECULE_FUNC_checkPointUpdate 8 //更新检查点
//分子效果
...
public:
    ...
    int i_curFrameTex;//当前用的是几号帧
    int i_foeFrameTex;//上一刻用的是几号帧，只在回滚模式中记录和使用
    float wDivh;//横纵比
    int funcID;//功效
    string startName;//传送起点名称
    string destinationName;//传送终点名称
    CtimeCounter revTimeCounter_restartTrans;//还差多少时间恢复传送
    int effectType;//效果类型
    ...
    void useTex(int i){//指定使用frameTexList中哪个帧来作为当前texture
        ...
        this->getModel()->getMeshByIndex(0)->setTexture(pFrameTexList[i%n]);//如果i越界则取余
    }
    void useLastTex(){
        int nTex=(int)pFrameTexList.size();
        useTex(nTex-1);
    }
    bool useNextFrameTex(){//使用下一帧，如果没有下一帧，则绕回，返回是否已播完此轮
        if(revTimeCounter_betweenFrame.getCount()!=0){//如果倒计时还没结束
            //继续倒计时，然后返回
            revTimeCounter_betweenFrame.doCount_dec();
            return false;
        }//如果倒计时已结束
        revTimeCounter_betweenFrame.setValue(timeCountBetweenFrame);//重设倒计时值
        ...
        this->getModel()->getMeshByIndex(0)->setTexture(pFrameTexList[i_curFrameTex]);
        return lastFrameOfCurRollFinished;
    }
    bool useNextFrameTex_notRewind(){//使用下一帧，如果没有下一帧，则停止在当前帧，返回是否已播完此轮
        if(revTimeCounter_betweenFrame.getCount()!=0){//如果倒计时还没结束
            //继续倒计时，然后返回
            revTimeCounter_betweenFrame.doCount_dec();
            return false;
    ...
    void useNextFrameTex_rollback(){//使用下一帧，如果没有下一帧，回滚
        if(revTimeCounter_betweenFrame.getCount()!=0){//如果倒计时还没结束
            //继续倒计时，然后返回
            revTimeCounter_betweenFrame.doCount_dec();
            return;
    ...
    void loadFrameTexList(const string&clipName){//clipName最后一个字符必须是"/"
        ...
        fclose(fp);
        //------------------获得clipName的最后一级名
        string clipNameLastSeg;//clipName的最后一级名
        string clipNameLastSeg_reverse;//clipNameLastSeg的逆序
  ...
  //      cout<<"hihi:"<<clipNameLastSeg<<endl;
        ...
        for(int i=0;i<nFrame;i++){
            string numStr=numberToStr(i+1);//注意，编号是从1开始的
