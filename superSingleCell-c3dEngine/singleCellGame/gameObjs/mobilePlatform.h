...
//运动类型
#define movType_STILL 0 //静止
#define movType_MOV 1 //平移
#define movType_ROT 2 //旋转
#define movType_MOV_AND_ROT 3 //旋转与平移混合
//触发类型
...
public:
  ...
  //  Cc3dALSource*pALSource_destroySound;
    ...
    bool motionSoundAfterTrigger_looping;//是否循环
public:
    vector<CIDTriangleEx*> pIDtriExList;//pmeshList中所有三角面统一索引到这个表里
public://为了优化而增加的结构
    int movType;//运动类型
    float bounceVup;//自动弹跳速度，如果为0则不弹跳
    bool moved;//是否发生了运动
public:
...
public:
    int ID;
    Cc3dMatrix4 initRMat;//初始旋转矩阵
    Cc3dVector4 startPos,endPos;//起始点
    float movSpeed;//移动速度
    Cc3dVector4 rot_n;//单位旋转轴
    float dA,cos_dA,sin_dA;//旋转角速度及其余弦和正弦
    string geoTypeName;//几何类型名
    float rx,rz,ry;//半长，半宽，半高
    string texImageName;//纹理图片名
    int triggerType_mov;//平移的触发类型
    int triggerType_rot;//旋转的触发类型
    bool triggered_mov;//平移是否已触发
    bool triggered_rot;//旋转是否已触发
    float rotAnglePerTrigger;//一次触发旋转的角度
    float angleRotedCurTrigger;//本次触发以来已旋过的角度
    Cc3dVector4 movVec;//实际移动向量
    bool isTrigger_movLocked;//移动触发锁定，不再触发移动
    bool isTrigger_rotLocked;//旋转触发锁定，不再触发旋转
    bool applyTrigger_movLock;//使用移动触发锁定
    bool applyTrigger_rotLock;//使用旋转触发锁定
    float initMovPercentage;//初始移动百分比
    float k_inertialVup;//竖直惯性速度系数
    bool destroyable;//可破坏
    Clife life;//生命值，当destroyable==true时有效
 
    bool isShotTrigger;//是否通过射击触发

public:
    float len2_startPos_endPos;//startPos与endPos之间距离的平方
    bool isMovingForth;//是否在前进(向endPos方向移动)
    Cc3dVector4 speedvec_forth;//前往移动速度向量
protected:
    Csprite*m_IDBoard0;//显示ID的公告牌，ID的第0位
    Csprite*m_IDBoard1;//显示ID的公告牌，ID的第1位
protected:
    float Rc;//碰撞半径
public:
    ...
    void updateNormsOfpIDtriExList(){//对pIDtriExList中和IDtriEx的法向量进行更新
        int nIDtriEx=(int)pIDtriExList.size();
        for(int i=0;i<nIDtriEx;i++){
            CIDTriangleEx*pIDtriEx=pIDtriExList[i];
            //v0,v1,v2的法向量都相等，都可以用作pIDtriEx的法向量
            int meshID=pIDtriEx->getMeshID();
            int vID=pIDtriEx->getIDtri().vID[0];
            const Cc3dVertex &v0=getModelByIndex(0)->getMeshByIndex(meshID)->getMeshData()->vlist[vID];
            pIDtriEx->setNorm(v0.getNorm().toV4(0));
        }
    ...
    void gen_pIDtriExList(){//生成pIDtriExList，要放在pmeshList全部形成后调用
        ...
        for(int i=0;i<nMesh;i++){
            Cc3dMesh&mesh=*getModelByIndex(0)->getMeshByIndex(i);
            ...
            for(int j=0;j<nIDtri;j++){
                ...
                //v0,v1,v2的法向量都相等，都可以用作pIDtriEx的法向量
                const Cc3dVertex &v0=mesh.getMeshData()->vlist[IDtri.vID[0]];
                pIDtriEx->setNorm(v0.getNorm().toV4(0));
    ...
    vector<CtriangleWithNorm> getCollisionTriList(){
        //将pIDtriExList转成triangleWithNormList并返回
        vector<CtriangleWithNorm> triWithNormList;
        {
            int nTri=(int)pIDtriExList.size();
            for(int i=0;i<nTri;i++){
                CIDTriangleEx*IDtriEx=pIDtriExList[i];
                const Cc3dVertex&v0=getModelByIndex(0)->getMeshByIndex(IDtriEx->getMeshID())->getMeshData()->vlist[IDtriEx->getIDtri().vID[0]];
                const Cc3dVertex&v1=getModelByIndex(0)->getMeshByIndex(IDtriEx->getMeshID())->getMeshData()->vlist[IDtriEx->getIDtri().vID[1]];
                const Cc3dVertex&v2=getModelByIndex(0)->getMeshByIndex(IDtriEx->getMeshID())->getMeshData()->vlist[IDtriEx->getIDtri().vID[2]];
                const Cc3dVector4&pos0=v0.getPos().toV4(1);
                const Cc3dVector4&pos1=v1.getPos().toV4(1);
                const Cc3dVector4&pos2=v2.getPos().toV4(1);
    ...
    void scaleToFitSurroundingBoxMatchingRxRyRz(){
        ...
        int nMesh=(int)getModelByIndex(0)->getMeshCount();
        for(int i=0;i<nMesh;i++){
            int nv=(int)getModelByIndex(0)->getMeshByIndex(i)->getMeshData()->vlist.size();
            for(int j=0;j<nv;j++){
                Cc3dVertex&v=getModelByIndex(0)->getMeshByIndex(i)->getMeshData()->vlist[j];
    ...
    void calMovType(){
        ...
        movType=movType_STILL;
        if(isEqual(startPos, endPos,0.01)//如果起止点重合
           ||movSpeed==0//或平移速度为0
           ){//没有平移
            //看是静止还是旋转
            if(dA==0){//静止
    ...
    void mov(){//transf中仍然保存位姿，但是在顶点却不是transf空间从标，而是世界空间坐标
        if(m_isDied)return;
        ...
        //----平移
        if(triggerType_mov!=triggerType_NONEEDTOTRIGGER&&triggered_mov==false){//需要触发却又还没触发
            //不平移
        }else{
            if(movType==movType_MOV||movType==movType_MOV_AND_ROT){
                ...
                if(isMovingForth){//如果在前进，则看是到startPos的距离平方是否大于len2_startPos_endPos
                    ...
                    if(len2_startPos_pos>=len2_startPos_endPos){//到达边界或越界
                        isMovingForth=false;//变向
                    ...
                    if(len2_endPos_pos>=len2_startPos_endPos){//到达边界或越界
                        isMovingForth=true;//变向
                        movVec=Cc3dVector4(0, 0, 0, 0)-speedvec_forth;//movVec=speedvec_forth
                        if(triggerType_mov!=triggerType_NONEEDTOTRIGGER){//如果需要触发，则此时已满一个周期，停止运动，等待下一次触发
                            triggered_mov=false;
                            movVec.init(0,0,0,0);
        ...
        //----旋转
        if(triggerType_rot!=triggerType_NONEEDTOTRIGGER&&triggered_rot==false){//需要触发却又还没触发
            //不旋转
        }else{
            if(movType==movType_ROT||movType==movType_MOV_AND_ROT){
                ...
                if(triggered_rot!=triggerType_NONEEDTOTRIGGER
                   &&angleRotedCurTrigger>=rotAnglePerTrigger){//如果需要触发且已转满规定的角度，停止
                    //将转过头的角度反向回转
                    {
                    ...
                    angleRotedCurTrigger+=fabsf(dA);//注意要用绝对值，因为dA可能为负
                }
            }
        }
        //声音停止
     ...
     //   cout<<"updateVertex"<<endl;
        ...
        for(int i=0;i<nMesh;i++){
            ...
            for(int j=0;j<nv;j++){
                Cc3dVertex&v=getModelByIndex(0)->getMeshByIndex(i)->getMeshData()->vlist[j];
                Cc3dVector3 pos=v.getPos();
                Cc3dVector3 norm=v.getNorm();
             //   Cc3dVertex&v_store=getModelByIndex(0)->getMeshByIndex(i)->vlist_store[j];
                Cc3dVertex&v_store=vlistStoreList[i][j];
                Cc3dVector3 pos_store=v_store.getPos();
                Cc3dVector3 norm_store=v_store.getNorm();
                //对v进行变换
                if(movType==movType_MOV){
                    ...
                    //坐标
                    Cc3dVector4 t_pos=m_transform.getRTmat()*pos_store.toV4(1);
                    pos.setx(t_pos.x());
                    pos.sety(t_pos.y());
                    pos.setz(t_pos.z());
                    v.setPos(pos);
                    //法线
                    Cc3dVector4 t_norm=m_transform.getRTmat()*norm_store.toV4(0);
                    norm.setx(t_norm.x());
                    norm.sety(t_norm.y());
                    norm.setz(t_norm.z());
                    v.setNorm(norm);
        ...
        //提交顶点
        for(int i=0;i<nMesh;i++){
            Cc3dMesh&mesh=*(this->getModelByIndex(0)->getMeshByIndex(i));
            mesh.submitVertex(GL_STREAM_DRAW);
        }
    ...
    void doTrigger_mov_ignoreTouchLocAndTriggerLock(){//触发mov，忽略触发位置和触发锁
        if(triggerType_mov==triggerType_NONEEDTOTRIGGER)return;//如果不需触发，则返回
        if(movType==movType_MOV||movType==movType_MOV_AND_ROT){
            bool triggered_mov_foe=triggered_mov;
            triggered_mov=true;
            //声音
            if(triggered_mov_foe==false){//之前是未trigger状态现转为triggered才放声音
    ...
    void doTrigger_rot_ignoreTouchLocAndTriggerLock(){//触发rot，忽略触发位置和触发锁
        if(triggerType_rot==triggerType_NONEEDTOTRIGGER)return;//如果不需触发，则返回
        if(movType==movType_ROT||movType==movType_MOV_AND_ROT){
            if(isTrigger_rotLocked==false){
                bool triggered_rot_foe=triggered_rot;
                triggered_rot=true;
                if(triggered_rot_foe==false){//之前是未trigger状态现转为triggered才放声音
                 //   pALSource_plat_motion->updatePosition(getTransform().getPos());
                 //   pALSource_plat_motion->play();
                    this->getALSourceByName("plat_motion")->updatePosition(getTransform().getPos());
                    this->getALSourceByName("plat_motion")->play();
...
public:
    ...
    float getH_floor(const Cc3dVector4&c,float Rc,Cc3dVector4& mobilePlatformSurfaceNorm,
                     CmobilePlatform*&pSurfaceMobilePlatform//注意要用指针的引用
                     )const {
        float floorHMax=-c3d_INF;//各mobilePlatform在c处的最大地面高度
        Cc3dVector4 floorNormMax(0,1,0,0);//最大地面高度处的法向
        CmobilePlatform*pmobilePlatformMax=NULL;
        int n=(int)actorList.size();
        for(int i=0;i<n;i++){
            CmobilePlatform*pmobilePlatform=(CmobilePlatform*)actorList[i];
            if(pmobilePlatform->getIsDied())continue;//跳过死亡的运动平台
            //求pmobilePlatform在c处表面高度和法向
            Cc3dVector4 norm;
            float H;
    ...
    float getH_ceil(const Cc3dVector4&c,float Rc,Cc3dVector4&mobilePlatformCeilNorm,
                    CmobilePlatform*&pCeilMobilePlatform//注意要用指针的引用
                    )const {
        float ceilHMin=c3d_INF;//各mobilePlatform在c处的最低天花板高度
        Cc3dVector4 ceilNormMin(0,-1,0,0);//最低天花板高度处的法向
        CmobilePlatform*pmobilePlatformMin=NULL;
        int n=(int)actorList.size();
        for(int i=0;i<n;i++){
            CmobilePlatform*pmobilePlatform=(CmobilePlatform*)actorList[i];
            if(pmobilePlatform->getIsDied())continue;//跳过死亡的运动平台
            //求pmobilePlatform在c处天花板高度和法向
            Cc3dVector4 norm;
            float H;
