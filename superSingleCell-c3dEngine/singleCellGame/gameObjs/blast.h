...
public:
    ...
    void layBlast(const Cc3dVector4&_pos,Cc3dCamera&camera,int blastType,
                  const Cc3dVector4&pcollisionFaceNorm=Cc3dVector4(0,0,0,0))const//在pos处放一个爆炸
    //collisionFaceNorm指明碰撞面的法向量
    {
        assert(_pos.w()==1);
        Cc3dVector4 pos=_pos;
        //在pBurstList中找一个died==true且blastType传入的balstType者
        Cblast*pblast=NULL;
        ...
        if(pblast){//如果找到了
            //放在pos处
            //为了避免公告牌被碰撞面裁切的问题，要将位置向视点拉近一些
            {
                //获得pblast的宽度
                float Rc=pblast->getRc();
                //将计算由pos向camera拉近rx后的位置
                const Cc3dVector4&eyePos=camera.getEyePos();
                //由pos到eyePos的向量
                Cc3dVector4 e_posToEyePos=eyePos-pos;
                e_posToEyePos=normalize(e_posToEyePos);
                if(isEqual(pcollisionFaceNorm,Cc3dVector4(0,0,0,0))==false){//如果提供了有反弹向量
                    //由e_posToEyePos,collsionFaceNorm和Rc计算最终位置
                    float cosA=dot(pcollisionFaceNorm, e_posToEyePos);
                    cosA=maxf(cosA, 0.5);//如果A小于60度则按60度算，这是为了防止cosA过小从而使下面计算出的pullBackLen过大
                    float pullBackLen=Rc/cosA;//沿e_posToEyePos拉回的距离
                    Cc3dVector4 pullBackVec;//拉近向量
                    pullBackVec=e_posToEyePos*pullBackLen;
                    pos=pos+pullBackVec;//此时pos为拉近后的结果
                }else{//如果没有提供反弹向量
                    //由e_posToEyePos和Rc计算最终位置
                    Cc3dVector4 pullBackVec;//拉近向量
                    pullBackVec=e_posToEyePos*Rc;
                    pos=pullBackVec+pos;//此时pos为拉近后的结果
                }
            }
            pblast->setPos(pos);
            pblast->adjustFace(camera.getEyePos());
            pblast->getModel()->getMeshByIndex(0)->submitVertex(GL_STREAM_DRAW);
