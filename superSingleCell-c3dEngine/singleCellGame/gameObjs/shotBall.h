...
protected:
    ...
    bool m_isActive;
public:
    string fileName_launchSound;//发射声音文件名
    string fileName_collisionWithSolidSound;//与固体碰撞声音文件名
    Cc3dVector4 posf;//前一时刻位置
    void*ptgBoss;//目标敌人
    bool tgBossIsFly;//目标敌人是否为飞行boss（仅在ptgBoss非空时有效）
    float pathLen;//记录从射出到当前走过的路程
    float shotBallReachDis;//子弹射程
    float vy0;//vy的初始速度
    float vy;//竖直速度
    float g;//重力加速度
    float hurt;//子弹的伤害
    float hurtDestroyablePlatform;//对运动平台的伤害
    int blastType;//本子弹的爆炸类型
...
public:
    ...
    void collisionWithGround(const Cground*ground,CblastSet&shotBlastSet,Cc3dCamera&camera){//与地面碰撞
        if(ground){
            Cc3dVector4 pos=m_transform.getPos();
    ...
    void collisionWithArch(CarchScene&archScene,CblastSet&shotBlastSet,Cc3dCamera&camera)
	{
        ...
        Cc3dVector4 pos=m_transform.getPos();//当前位置
        //判断线段posf--pos是否与arch相交
        Cc3dVector4 p;//交点坐标
