...
public:
    ...
    int templateID;//模板ID
    float MHD_fromMyCell;//与主角的曼哈顿距离
    bool midFar;//中等远
    bool tooFar;//太远
    Cc3dRange flyRange_relativeToInitPos;//相对于初始位置的飞行范围
    Cc3dRange flyRange;//飞行范围
    CtimeCounter revTimeCounter_noHert;//重生后从无伤害到有伤害倒计时
    CtimeCounter revTimeCounter_headingToTargetAgain;//再次向目标前进倒计时
    long afterHowLongHeadingToTargetAgain;//多长时间后再次向目标前进
    bool initiativeAttack;//是否主动攻击
    ...
    void loadConfigAndMeshes(const string&configPathShort,const string&modelClipPathShort){
        ...
        //------------------提取文件信息，并建立模型列表
        {
            Cc3dModel *model=new Cc3dModel();//临时模型
            model->autorelease();
            char buffer[1024]={0};
            //提取modelScale
            fscanf(fp, "%s",buffer);
            ...
            fscanf(fp, "%d",&nMesh);
            //加载模型
            loadInfo_model(model, modelClipPathShort,this->modelScale,this->modelOffsetVec);
            if((int)model->getMeshCount()!=nMesh){
            ...
            for(int i=0;i<nMesh;i++){
                ...
                //生成mesh，拷贝tmodel.pmeshList[meshID]
                Cc3dMesh*pmesh=model->getMeshByIndex(meshID);
                //将pmeshList从模型中取下，但不删除pmeshList元素
                pmesh->retain();
                pmesh->removeFromParent();
                //将pmesh添加到pModelBaseList[cellllPartID]这个模型中
                getModelByIndex(cellPartID)->addMesh(pmesh);
            }
            //为各模型genVBOBuffers
            {
                int nModel=(int)getModelCount();
                for(int i=0;i<nModel;i++){
                    Cc3dModel*pmodel=getModelByIndex(i);
                    if(pmodel==NULL)continue;
                    pmodel->genVBOBuffers();
                }
            }
            //提取各cellPart的材质参数，并填充到model
            for(int i=0;i<CELLPARTCOUNT;i++){
                ...
                //填充到model
                if(diffuse[0]!=-1){
                    pmodel->setDiffuseAlpha(diffuse[3]);
                    pmodel->setDiffuseRGB(diffuse[0], diffuse[1], diffuse[2]);
                }
                if(ambient[0]!=-1)pmodel->setAmbient(ambient[0], ambient[1], ambient[2]);
                if(specular[0]!=-1)pmodel->setSpecular(specular[0], specular[1], specular[2]);
                if(shininess!=-1)pmodel->setShininess(shininess);
                if(isHighlightUntransp!=-1)pmodel->setIsHighlightUntransp(isHighlightUntransp);
            }
            //提取其余参数
    ...
    //targetPos为this的攻击目标z位置
    {
        ...
        if(v_forward_roll_value!=0){
            this->getModelByIndex(CELLPART_body)->getTransformPointer()->rotate(1, 0, 0, cos_v_forward_roll_value, -sin_v_forward_roll_value);
        }
    ...
    //必须放在着陆之后，否则容易发生与地面之下的墙面进行误碰撞
    {
        ...
        if(d>0){
            ...
            Cc3dVector4 back;
            float d_back=d*0.35;//0.25;//d*0.1;//0.25;//0.15;//除了恢复原位外另加一个回退
            back=e_back*d_back;
            back.sety(0);
...
public:
    ...
    Cc3dRange getBossShipFlyRange(){//获得所有bossShip的飞行范围的并
