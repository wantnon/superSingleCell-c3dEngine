...
public:
    bool isMoving;//是否在移动
    Ccur_foe<bool> isCollisionWithArchWall_curFoe;//本刻和上一刻是与arch墙壁撞击
    Ccur_foe<bool> isCollisionWithMobilePlatformWall_curFoe;//本刻和上一刻是否与运动平台墙壁撞击
    Ccur_foe<bool> isCollisionWithArchCeil_curFoe;//本刻和上一刻是与arch天花板撞击
    Ccur_foe<bool> isCollisionWithMobilePlatformCeil_curFoe;//本刻和上一刻是否与运动平台天花板撞击
    string fileName_engineSound;//引擎音效文件名
    string fileName_collisionSound;//碰撞声音
    CtimeCounter revTimeCounter_headingToTargetAgain;//再次向目标前进倒计时
    long afterHowLongHeadingToTargetAgain;//多长时间后再次向目标前进
...
public:
    ...
    float v_pushForward_value;//推动前进速度值
    float v_pushBack_value;//推动后退速度值
    float v_pushUp_value;//推动上升速度值
    float v_pushDown_value;//推动下降速度值
    float a_vyPushDec;//推动竖直速度衰减
    float a_vPushForwardDec;//推动前进速度衰减
    float Rc_getOnShip;//上船需要的碰撞半径
    Cc3dVector4 seatPos_local;//座位位置（局部坐标），主角上船后中心处于此位置
    Cc3dVector4 up_disturbed;//扰动过的up向量
    bool isFlyable;//是否可飞行
    Cc3dMatrix4 RTmat_store_beforeFloating;//在漂行之前保存的RTmat
    float modelScale;//模型缩放比例
    //   Cc3dVector4 modelOffsetVec;//模型偏移向量
    Cship(){
    ...
    void autoMov(const Cc3dVector4&targetPos){//targetPos为this的攻击目标z位置
        if(initiativeAttack&&revTimeCounter_headingToTargetAgain.getCount()==0){//主动攻击为true且倒计时结束才重新朝向目标
            //求与由this指向主角的单位向量
    ...
    void adjustPosToCeil(float softness=0.2){
        ...
        if(collisionWithCeil){
            ...
            //如果高度高于wantedHeight，则调整为wantedHeight,如果低于，则不管，这样可以避免粘黏
            if(m_transform.getRTmat().getAt(13)>wantedHeight){
                m_transform.getRTmatPointer()->setAt(13, wantedHeight);
            }
            //声音
    ...
    Cc3dMatrix4 getSeatRTmat()const {
        ...
        //用seatPos_world替换seatRTmat的平移分量
    ...
    bool collisionAndReActWithArchWall(CarchScene&archScene,bool multiPoint,const vector<int>&skipTagList,const int intended_nSP){
        ...
        //碰撞扰动
        if(isCollisionWithArchWall_curFoe.isChanged()&&isCollisionWithArchWall_curFoe.get()==true){//如果isCollisionWithArch_curFoe发生变化且是由false变为true，则播放碰撞声音
            if(isFlyable){//只对会飞行的ship调整朝向，船只不调
                //调整朝向
                Cc3dVector4 wantedDir=Cc3dVector4(0,0,0,0)-e_back;
                if(isFlyable==false)wantedDir.sety(0);//如果不能飞，则舍弃Y分量
    ...
    //将this囚禁在以c为中心R为边心距的cube内
    //前端返回是否出界
    {
        bool isOutRange=false;//是否出界
        //求cube范围
    ...
    bool collisionAndReActWithmobilePlatformWall(CmobilePlatformSet&mobilePlatformSet,bool triggerCollisionPlatforms,bool multiPoint,const int intended_nSP){
        ...
        //声音
        if(isCollisionWithMobilePlatformWall_curFoe.isChanged()&&isCollisionWithMobilePlatformWall_curFoe.get()==true){//如果isCollisionWithMobilePlatform_curFoe发生变化且是由false变为true，则播放碰撞声音
        //    pALSource_ship_collision->updatePosition(getTransform().getPos());
        //    pALSource_ship_collision->playIfNotPlaying();
            if(this->getALSourceByName("ship_collision")){
            this->getALSourceByName("ship_collision")->updatePosition(getTransform().getPos());
    ...
    //前端返回本次由自身动力引起的移动路程
 	{
        ...
        //--前进
        this->m_transform.moveZ(-vforward_push);//注意，这句不只是水平移动，也可能包含了竖直方向上的移动
    ...
    //triggerPlatform指示是否触发移动平台
    {
        ...
        if(isTouchSurface(margin)){//接触表面
            ...
            if(m_transform.getRTmat().getAt(13)<wantedHeight){//如果小于就拉上来，否则不影响，这样能避免粘黏
                m_transform.getRTmatPointer()->setAt(13, wantedHeight);
            }
            //调整姿态
...
public:
  ...
  /*  void show(const Ccamera&camera,const Clight&light,const Cprogram&program){
        ...
        for(int i=0;i<n;i++){
            ...
            if(pship->visible){
                  pship->getModelList()[0]->show_pmeshList(camera, light, program,&pship->transf);
            }
        }
