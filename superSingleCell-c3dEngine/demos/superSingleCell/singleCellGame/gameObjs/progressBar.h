...
public:
    ...
    void genAndInit(float bx,float by,float width,float height){
        ...
        m_quad_back->genAndInit(bx, bx+width, by, by+height,
                          -0.04,1.04,-0.04,1.04//注意iphone中t轴向下。另外为了边框能完整显示，使用纹理坐标缩放的技巧
                            );
    ...
    void update(float _percentage,GLenum usage){
        ...
        //根据percentage计算新的宽度，更新quad_front的宽度
        m_quad_front->reSetWidth((m_quad_back->xmax-m_quad_back->xmin)*percentage);
        //重新提交顶点
        m_quad_front->getModel()->getMeshList()[0]->getSubMeshByIndex(0)->getIndexVBO()->submitVertex(m_quad_front->getModel()->getMeshList()[0]->getSubMeshByIndex(0)->getSubMeshData()->vlist,usage);
    }
