...
public:
    ...
    bool init(float _rx,float _ry,float sRepeat=1,float tRepeat=1){
        ...
        subMesh->autorelease();
        subMesh->init();
        subMesh->getSubMeshData()->vlist.resize(nPoint);
        memset(&(subMesh->getSubMeshData()->vlist[0]),0,sizeof(Cc3dVertex)*nPoint);//清零
        Cc3dIDTriangle IDtri0,IDtri1;
        IDtri0.setvID(0, 1, 2);
        IDtri1.setvID(0, 2, 3);
        subMesh->getSubMeshData()->IDtriList.push_back(IDtri0);
        subMesh->getSubMeshData()->IDtriList.push_back(IDtri1);
        Cc3dVertex&v0=subMesh->getSubMeshData()->vlist[0];
        Cc3dVertex&v1=subMesh->getSubMeshData()->vlist[1];
        Cc3dVertex&v2=subMesh->getSubMeshData()->vlist[2];
        Cc3dVertex&v3=subMesh->getSubMeshData()->vlist[3];
    ...
    void adjustFace(const Cc3dVector4&eyePos){//调整朝向(旋转中心在中心)
        ...
        Cc3dVertex&v0=this->getMesh()->getSubMeshByIndex(0)->getSubMeshData()->vlist[0];
        Cc3dVertex&v1=this->getMesh()->getSubMeshByIndex(0)->getSubMeshData()->vlist[1];
        Cc3dVertex&v2=this->getMesh()->getSubMeshByIndex(0)->getSubMeshData()->vlist[2];
        Cc3dVertex&v3=this->getMesh()->getSubMeshByIndex(0)->getSubMeshData()->vlist[3];
        v0.setPos(p0.toV3());
        v1.setPos(p1.toV3());
        v2.setPos(p2.toV3());
        v3.setPos(p3.toV3());
