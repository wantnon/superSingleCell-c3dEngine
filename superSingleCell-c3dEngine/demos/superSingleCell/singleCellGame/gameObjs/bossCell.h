...
public:
    ...
    void loadConfigAndMeshes(const string&configPathShort,const string&modelClipPathShort){
        ...
        //------------------提取文件信息，并建立模型列表
        {
            Cc3dMesh *mesh=new Cc3dMesh();//临时模型
            mesh->autorelease();
			mesh->init();
            char buffer[1024]={0};
            //提取modelScale
    ...
    //targetPos为this的攻击目标z位置
    {
        ...
        if(v_forward_roll_value!=0){
            this->getModel()->getMeshByIndex(CELLPART_body)->getTransformPointer()->rotate(1, 0, 0, cos_v_forward_roll_value, -sin_v_forward_roll_value);
        }
