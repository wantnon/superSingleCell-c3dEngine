...
public:
    ...
    void updateNormsOfpIDtriExList(){//对pIDtriExList中和IDtriEx的法向量进行更新
        ...
        for(int i=0;i<nIDtriEx;i++){
            ...
            const Cc3dVertex &v0=getModel()->getMeshByIndex(0)->getSubMeshByIndex(meshID)->getSubMeshData()->vlist[vID];
    ...
    void gen_pIDtriExList(){//生成pIDtriExList，要放在pmeshList全部形成后调用
        ...
        for(int i=0;i<nSubMesh;i++){
            ...
            for(int j=0;j<nIDtri;j++){
                ...
                //v0,v1,v2的法向量都相等，都可以用作pIDtriEx的法向量
                const Cc3dVertex &v0=submesh.getSubMeshData()->vlist[IDtri.vID[0]];
                pIDtriEx->setNorm(v0.getNorm().toV4(0));
    ...
    vector<CtriangleWithNorm> getCollisionTriList(){
        ...
        vector<CtriangleWithNorm> triWithNormList;
        {
            ...
            for(int i=0;i<nTri;i++){
                ...
                const Cc3dVertex&v0=getModel()->getMeshByIndex(0)->getSubMeshByIndex(IDtriEx->getSubMeshID())->getSubMeshData()->vlist[IDtriEx->getIDtri().vID[0]];
                const Cc3dVertex&v1=getModel()->getMeshByIndex(0)->getSubMeshByIndex(IDtriEx->getSubMeshID())->getSubMeshData()->vlist[IDtriEx->getIDtri().vID[1]];
                const Cc3dVertex&v2=getModel()->getMeshByIndex(0)->getSubMeshByIndex(IDtriEx->getSubMeshID())->getSubMeshData()->vlist[IDtriEx->getIDtri().vID[2]];
    ...
    void scaleToFitSurroundingBoxMatchingRxRyRz(){
        ...
        int nSubMesh=(int)getModel()->getMeshByIndex(0)->getSubMeshCount();
        for(int i=0;i<nSubMesh;i++){
            int nv=(int)getModel()->getMeshByIndex(0)->getSubMeshByIndex(i)->getSubMeshData()->vlist.size();
            for(int j=0;j<nv;j++){
                Cc3dVertex&v=getModel()->getMeshByIndex(0)->getSubMeshByIndex(i)->getSubMeshData()->vlist[j];
