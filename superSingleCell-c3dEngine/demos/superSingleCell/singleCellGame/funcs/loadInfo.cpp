...
//由于是等比放缩，所以法线和纹理坐标不受影响
{
    ...
    //------------------提取文件信息到submesh
    {
        ...
        int nv;
        fscanf(fp, "%d",&nv);
        submesh->getSubMeshData()->vlist.reserve(nv);
        //提取各顶点
        for(int i=0;i<nv;i++){
            ...
            //将此顶点加入vlist
            submesh->getSubMeshData()->vlist.push_back(vertex);
        }
        //提取IDtri数
        int nIDtri;
        fscanf(fp, "%d",&nIDtri);
        submesh->getSubMeshData()->IDtriList.reserve(nIDtri);
        ...
        for(int i=0;i<nIDtri;i++){
            ...
            submesh->getSubMeshData()->IDtriList.push_back(IDtri);
        }
        //提取纹理图片名并加载纹理
        {
...
void loadInfo_mesh(Cc3dMesh*mesh,const string&modelClipPathShort,float scale,Cc3dVector4 offsetVec){//输入模型文件夹短路径名
