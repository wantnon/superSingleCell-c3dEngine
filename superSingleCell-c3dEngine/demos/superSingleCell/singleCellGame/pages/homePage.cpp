...
bool ChomePage::init(){
    ...
    //--quad_homePage_backGround
    {
        ...
        m_quad_homePage_backGround->init();
...
void ChomePage::update(){
    ...
    //----gameKey
    {
        ...
        for(int i=0;i<nKey;i++){
            ...
            if(CuserConfig::sharedUserConfig()->unLockList[i]==false&&CgameSettings::sharedGameSettings()->getUnlockAll()==false)continue;//跳过未解锁的关
            //key响应单击
            if(i<CuserConfig::sharedUserConfig()->get_levelCount()){
                if(key->isContainPoint(Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getPoint())&&Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getIsTapOnce()){
                    //curlevel=i+1;
            ...
            //key响应按下和抬起
            if(Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getIsDown()&&key->isContainPoint(Cc3dGestureAnalyzer::sharedGestureAnalyzer()->getPoint()) ){
                key->useKeyDownColor();
            }else{
                key->useKeyUpColor();
            }
