...
bool  CplayingPage::init(int level){
  ...
  //  assert(cell->getFather()==NULL);
    ...
    //--archScene
    {
        ...
        m_archScene->getMesh()->setColor(1, 1, 1, 0.7);//设置透明度，但在show_solid时不开启混合，在show_transp时开启混合
    ...
    //--mobilePlatformSet
    {
        ...
        for(int i=0;i<nPlat;i++){
            ...
            if(pmobilePlatform->geoTypeName=="cube_program"){//程序生成cube
                ...
                subMesh->init();
                subMesh->autorelease();
                pmobilePlatform->getMesh()->addMesh(subMesh);
       ...
       //         subMesh->setMeshData());
                //存储顶点
                pmobilePlatform->vlistStoreList.push_back(subMesh->getSubMeshData()->vlist);
       //        subMesh->store_vlist();
                //生成vbo缓冲并提交顶点
...
//------------------------------------------------------------------------------------------------------------------------------------
    ...
    //--quad_halfFuelHaveBeenUsed
    {
		...
		m_quad_halfFuelHaveBeenUsed->init();
        //
        float bx=0;
        float by=0;
        float w=200;
