...
static inline bool getPointFootOnTri(const Cc3dVector4&p,
                       const Cc3dVector4&p0,const Cc3dVector4&p1,const Cc3dVector4&p2,const Cc3dVector4&norm,//norm为三角面p0p1p2的法向量
                       Cc3dVector4&foot
...
static inline bool getPointFootOnLine(const Cc3dVector4&p,const Cc3dVector4&p0,const Cc3dVector4&p1,Cc3dVector4&foot){//求点p到直线p0p1的垂足，前端返回垂足是否落在线段p0p1上
    assert(p.w()==1);
    assert(p0.w()==1);
    assert(p1.w()==1);
    ...
    Cc3dVector4 e_p0p1=normalize(p0p1);//p0p1的单位向量
    Cc3dVector4 p0foot;
    float t=dot(p0p,e_p0p1);//p0沿p0p1到foot的有向长度
    p0foot=e_p0p1*t;
...
static inline float minDis_point_Tri(const Cc3dVector4&point,
                       const Cc3dVector4&p0,const Cc3dVector4&p1,const Cc3dVector4&p2,const Cc3dVector4&norm,//norm为三角面p0p1p2的法向量
                       Cc3dVector4&minp){//求点point到三角面p0p1p2的最短距离，前端返回最小距离值，minp带回取得最小距离的点
    //----制作关键点列表和有效性列表
    Cc3dVector4 p[7];//关键点列表（考察的7个关键点是：point到三角面垂足，point到三角面的三条边的垂足，三角面三个顶点）
    bool valid[7];//垂足的有效性列表
    //point在平面上的垂足
    valid[0]=getPointFootOnTri(point,p0,p1,p2,norm,p[0]);
    if(valid[0]==true){//如果垂足在三角面内，则此必为最短距离，不用再往下算，此为一优化
...
//有无穷多交点时p3直接等于p1
{
    ...
    Cc3dVector4 v=normalize(p2-p1);//直线的单位方向向量
...
//求空间点c到射线(p,dir)的距离的平方，如果点到射线所在直线的垂足落在射线外，则规定距离为无穷
{
    Cc3dVector4 pc=c-p;
    if(dot(dir, pc)<0){
        return c3d_INF;
